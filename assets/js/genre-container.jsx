...
class GenreContainer extends Component {
    ...
    getExploreName = () => {
        let title = this.props.genreInfo.activeGenre.toLowerCase();
        let exempt = [
                "comedy",
                "drama",
                "documentary",
    ...
    handleResize = () => {
        if (this.movieListRef.current) {
            if (this.props.genreInfo.movies) {
                if (
                    this.props.genreInfo.movies.length * this.itemWidth >
    ...
    shouldComponentUpdate(nextProps, nextState) {
        if (
            ...
            nextState.showExplore === this.state.showExplore &&
            nextState.exploreName === this.state.exploreName &&
            nextState.hideMovies === this.state.hideMovies
        ) {
            return false;
        } else {
            return true;
    ...
    render() {
        let movies = this.props.genreInfo.movies
            .slice(0, 20)
            .map((movie, index) => (
                <MovieItem
            ...
            >
                <Fade distance="20%" bottom>
                    <div
                        ...
                        style={{
                            width: `calc(100% + ${
                                this.state.movieScrollLeft ? "40px" : "20px"
                            })`,
                        }}
                    >
                        <div
                        ...
                        {this.state.showArrows ? (
                            ...
                            >
                                <Fade
                                    duration={350}
                                    when={this.state.movieScrollLeft}
                                    distance="10%"
                                    left
                                >
                                    <div
                                ...
                                <Fade
                                    duration={350}
                                    when={this.state.movieScrollRight}
                                    distance="10%"
                                    right
                                >
                                    <div
