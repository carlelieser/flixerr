...
class MovieItem extends Component {
    ...
    loadImage = () => {
        if (!this.unMounting) {
            this.setState({
                backdrop: `${
                    this.props.shows
                        ? this.props.movie.flixerr_data.series_backdrop_path
                        : this.props.fallback
    ...
    handleImage = () => {
        let posterImage = this.createNewImage(
            this.props.movie.flixerr_data.poster_path
        );
        let colorImage = this.createNewImage(
            this.props.movie.flixerr_data.blurry_backdrop_path
        );
        let fac = new FastAverageColor();

        fac.getColorAsync(colorImage, { algorithm: "sqrt" })
            .then((color) => {
                if (!this.unMounting) {
        ...
        if (this.props.fallback) {
            let backdropImage = this.createNewImage(
                this.props.movie.flixerr_data.backdrop_path
            );
    ...
    shouldComponentUpdate(nextProps, nextState) {
        if (
            nextState.averageColor === this.state.averageColor &&
            nextState.backdrop === this.state.backdrop &&
            nextState.fontSize === this.state.fontSize &&
            nextState.stars === this.state.stars &&
            ...
            <Fade distance="10%" bottom>
                <div className="movie-item" onClick={this.handleMovieClick}>
                    ...
                    <img
                        ...
                        src={
                            this.props.fallback || this.props.shows
                                ? this.props.movie.flixerr_data
                                      .blurry_backdrop_path
                                : this.props.movie.flixerr_data
                                      .blurry_poster_path
                        }
